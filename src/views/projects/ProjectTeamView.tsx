...
export default function ProjectTeamView () {
  ...
  const removeUser = (userId:TeamMember['_id']) => {
    const data = {
      userId,
      projectId
    }
  ...
  if (data) return (
    <>
        <h1 className=' text-5xl font-bold'>Administra equipo</h1>
        <p className=' text-2xl font-light text-gray-500 mt-5' >Administra el equipo de trabajo para este projecto</p>
        <nav className='my-5 flex gap-3'>
            <button
                type='button'
                className='bg-purple-400 hover:bg-purple-500 px-10 py-3 text-xl font-semibold cursor-pointer
                text-white transition-colors'
                onClick={() => navigate('?addMember=true')}
            >Agregar colaborador</button>
            <Link to={`/projects/${projectId}`} className='bg-fuchsia-600 hover:bg-fuchsia-700 px-10 py-3 text-xl font-semibold cursor-pointer
                text-white transition-colors'>
              Volver a projecto
            </Link>
        </nav>
